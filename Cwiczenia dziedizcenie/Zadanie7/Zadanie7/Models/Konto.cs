...
namespace Zadanie7.Models
{
    internal class Konto
    {
        ...
        public Konto(int numerKonta, string daneWlasciciela, DateTime dataUtworzenia)
        {
            ...
            transakcje = new Transakcja[2][]; // tablica seregow [] to tablica 2 wymiarowa tablic 1 wymiarowych 
        ...
        public void wyswietlZTransakcjami()
        {
            Console.WriteLine("Transakcje na koncie: ");
            for (int i = 0; i < liczbaUznan; i++)
            {
                var transakcja = transakcje[0][i];
                transakcja.WypiszInfoTransakcji();

                if (transakcja is Przelew przelew)
                {
                    Console.WriteLine($"Przelew z konta: {przelew.KontoZrodlowe.NrKonta} do: {przelew.KontoDocelowe.NrKonta}");
                }
            }

            for (int i = 0; i < liczbaObciazen; i++)
            {
                var transakcja = transakcje[1][i];
                transakcja.WypiszInfoTransakcji();
